...
const makeAuth = ({ meCall, loginCall, refreshTokenCall }) => {
  ...
  function* lsGetAccessToken() {
    return yield call(
      ...
      `${STORAGE_KEY_NS}:accessToken`
    );
  }
  function* lsGetRefreshToken() {
    return yield call(
      [window.localStorage, "getItem"],
      `${STORAGE_KEY_NS}:refreshToken`
    );
  }
  ...
  function* authenticateWithStorageToken() {
    ...
    if (lsAccessToken) {
      ...
      try {
        ...
        const [user, { accessToken, refreshToken }] = yield apiCallWithRefresh(
          lsAccessToken,
          lsRefreshToken,
          me
        );
