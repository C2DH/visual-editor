...
class DocumentChooser extends PureComponent {
  ...
  getFilterParams = (searchString) => {
    ...
    if (this.props.withPlaceTypeFilters) {
      const placeTypes = Object.keys(this.state.placeTypeFilters).filter(
        (placeType) => this.state.placeTypeFilters[placeType]
      );
  ...
  render() {
    ...
    return (
      <Container fluid className="margin-r-l-20">
        ...
        <div
          className={`DocumentChooser__List${
            withPlaceTypeFilters ? " wPlaceFilters" : ""
          }`}
        >
          {documents && (
            <Row>
          ...
          {canLoadMore && count > 0 && !loading && (
            <div className="DocumentChooser__LoadMoreBtn">
        ...
        <div className="DocumentChooser__BottomContent">
          <Row>
            {multi && (
              <Col md={3}>
                <Button size="sm" block onClick={selectionDone}>
                  Done
                </Button>
              </Col>
            )}
            <Col md={3}>
              <Button size="sm" block onClick={this.props.hideWidgetFullPage}>
                Exit
              </Button>
            </Col>
            <Col md={multi ? 6 : 9} />
